...
async def list_threads_view_whth_llm(
    *,
    llm_id: str,
    db: AsyncSession,
    current_user_id: str,
    ...
    if not llm_id:
        raise HTTPException(status_code=422, detail="llm_id 不能为空")

    conv_repo = ConversationRepository(db)
    conversations = await conv_repo.list_conversations_with_llm(
        user_id=str(current_user_id),
        llm_id=llm_id,
        status="active",
        limit=limit,
