...
import LLMChatComponent from '@/components/LLMChatComponent.vue'
// import LLMConfigSidebar from '@/components/LLMConfigSidebar.vue'
...
// 当前选中的模型
const currentModelSpec = ref('')

// 处理模型选择
const handleModelSelect = (modelSpec) => {
  currentModelSpec.value = modelSpec
  localStorage.setItem('last_selected_llm_model', modelSpec)
  message.success(`已切换到模型: ${modelSpec}`)
}

onMounted(() => {
  const savedModel = localStorage.getItem('last_selected_llm_model')
  if (savedModel) {
    currentModelSpec.value = savedModel
  }
})
