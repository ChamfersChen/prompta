...
export const llmApi = {
   ...
   */
  sendLlmMessage: (llmId, data, options = {}) => {
    ...
    return fetch(`/api/llm/llm/${llmId}`, {
      method: 'POST',
   ...
   */
  simpleCall: (query) => apiPost('/api/llm/call', { query }),
   ...
   */
  getLlmHistory: (threadId) =>
    apiGet(`/api/llm/message/history?thread_id=${threadId}`),
   ...
   */
  getProviderModels: (provider) => apiGet(`/api/llm/models?model_provider=${provider}`),
  ...
  updateProviderModels: (provider, models) =>
    apiPost(`/api/llm/models/update?model_provider=${provider}`, models),
...
export const llmThreadApi = {
  ...
  getThreads: (agentId, limit = 100, offset = 0) => {
    const url = `/api/llm/threads?llm_id=${agentId}&limit=${limit}&offset=${offset}`
    return apiGet(url)
  },

  /**
   * 创建新对话线程
   * @param {string} llmId - 智能体ID
   * @param {string} title - 对话标题
   * @param {Object} metadata - 元数据
   * @returns {Promise} - 创建结果
   */
  createThread: (llmId, title, metadata) =>
    apiPost('/api/llm/thread', {
      llm_id: llmId,
      title: title || '新的对话',
      metadata: metadata || {}
    }),
  ...
  updateThread: (threadId, title, is_pinned) =>
    apiPut(`/api/llm/thread/${threadId}`, {
      title,
      is_pinned
    }),
   ...
   */
  deleteThread: (threadId) => apiDelete(`/api/llm/thread/${threadId}`),
